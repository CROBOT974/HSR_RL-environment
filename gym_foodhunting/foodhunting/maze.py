...
class MazeEnv(gym.Env):
    ...
    def reset(self):
        ...
        wall_id = p.loadURDF('maze.urdf')
        object_id = p.loadURDF('food_sphere.urdf', [0, 1.6, 0.5], globalScaling=self.object_size)
        ...
        for i in range(self.BULLET_STEPS):
            ...
            p.resetDebugVisualizerCamera(
                ...
                cameraYaw=90,  # 180
                cameraPitch=-80,  # -80
                cameraTargetPosition=[-1, 1, 1])  # 0,1.3,1
    ...
    def _getReward(self):
        ...
        contacted_wall_ids = [wall_id for wall_id in self.wall_ids if self.robot.isContact(wall_id)]
        for wall_id in contacted_wall_ids:
            reward -= 0.002
        return reward
    ...
    def _isDone(self):
        ...
        """
        available_object_ids = [object_id for object_id in self.object_ids]
        available_wall_ids = [wall_id for wall_id in self.wall_ids]

        return self.steps >= self.max_steps or len(available_object_ids) <= 0 or len(available_wall_ids) <= 0
