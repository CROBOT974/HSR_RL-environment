...
class Robot:
    ...
    def __init__(self, urdfPath=URDF_PATH, position=[0.0, 0.0, 1.0], orientation=[0.0, 0.0, 0.0, 1.0]):
        ...
        _ = p.loadURDF("wall.urdf", basePosition=[0, 0, 0], useMaximalCoordinates=True)
...
class HSR_FH(Robot):
    ...
    def __init__(self, urdfPath=URDF_PATH, position=[0.0, 0.0, 0.05], orientation=[0.0, 0.0, 0.0, 1.0]):
        ...
        """
        super(HSR_FH, self).__init__(urdfPath, position, orientation)
...
class FoodHuntingEnv(gym.Env):
    ...
    def __init__(self, render=False, robot_model=HSRSimple_FH, max_steps=500, num_foods=3, num_fakes=0, object_size=1.0, object_radius_scale=1.0, object_radius_offset=1.0, object_angle_scale=1.0):
    ...
    def reset(self):
        ...
        for i in range(self.BULLET_STEPS):
            ...
            p.resetDebugVisualizerCamera(
                ...
                cameraYaw=90,  # 180
                cameraPitch=-80,  # -80
                cameraTargetPosition=[1, 0, 1])  # 0,1.3,1
    ...
    def _generateObjectPositions(self, num=1, retry=100, radius_scale=1.0, radius_offset=1.0, angle_scale=1.0, angle_offset=0.5*np.pi, z=2, near_distance=0.5):
